...
def get_config_value(
    ...
    """
    Read a configuration value from Streamlit Secrets when deployed.

    During local development, fall back to values loaded from the .env file.
    """
    try:
        ...
        # st.secrets may be unavailable when no local secrets.toml exists.
        pass

    return os.getenv(name, default)


GOOGLE_MAPS_API_KEY = get_config_value(
    "GOOGLE_MAPS_API_KEY"
)
...
# Used only for local development. On Streamlit Community Cloud, the service
# account is read from the [gcp_service_account] section of st.secrets.
...
def has_streamlit_service_account() -> bool:
    """Return True when Streamlit Secrets contains Google service credentials."""
    try:
...
def get_google_cloud_credentials():
    """
    Return Google Cloud credentials for the current environment.

    Streamlit Cloud:
        Build credentials from the [gcp_service_account] secret section.

    Local development:
        Return None so google-cloud-vision uses Application Default
        Credentials through GOOGLE_APPLICATION_CREDENTIALS.
    """
    if not has_streamlit_service_account():
        return None
    ...
    try:
        ...
        raise RuntimeError(
            "The [gcp_service_account] values in Streamlit Secrets "
...
def validate_environment() -> list[str]:
    ...
    streamlit_credentials_available = has_streamlit_service_account()
    local_credentials_path = GOOGLE_APPLICATION_CREDENTIALS

    if local_credentials_path:
        credential_path = Path(local_credentials_path)

        if not credential_path.exists():
            missing.append(
                "The local Google service-account file does not exist: "
                f"{local_credentials_path}"
            )

    elif not streamlit_credentials_available:
        missing.append(
            "Google Cloud Vision credentials are missing. "
            "Use GOOGLE_APPLICATION_CREDENTIALS locally or add "
...
if missing_configuration:
    ...
    st.info(
        "For local development, add the required values to .env. "
        "For Streamlit Community Cloud, add them under App settings → Secrets."
    )
